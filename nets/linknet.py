...
class BasicBlockms(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, groups=1, bias=False):
        super(BasicBlock, self).__init__()
        self.conv1 = conv_3_1(in_planes, out_planes, kernel_size, stride, padding, groups=groups, bias=bias)
        self.bn1 = nn.BatchNorm2d(out_planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv_3_1(out_planes, out_planes, kernel_size, 1, padding, groups=groups, bias=bias)
        self.bn2 = nn.BatchNorm2d(out_planes)
        self.downsample = None
        if stride > 1:
            self.downsample = nn.Sequential(nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False),
...
class Encoderms(nn.Module):

    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, groups=1, bias=False):
        super(Encoderms, self).__init__()
        self.block1 = BasicBlock(in_planes, out_planes, kernel_size, stride, padding, groups, bias)
        self.block2 = BasicBlock(out_planes, out_planes, kernel_size, 1, padding, groups, bias)
...
class MSLinkNetBase(nn.Module):
    ...
    def __init__(self, n_classes=2):
        ...
        :type x_n: int
        """
        super(MSLinkNetBase, self).__init__()
        ...
        self.encoder1 = Encoderms(64, 64, 3, 1, 1)
        self.encoder2 = Encoderms(64, 128, 3, 2, 1)
        self.encoder3 = Encoderms(128, 256, 3, 2, 1)
        self.encoder4 = Encoderms(256, 512, 3, 2, 1)
