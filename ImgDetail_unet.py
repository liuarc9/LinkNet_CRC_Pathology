...
def strip2Img(multiArg, q):
    ...
    wholePatch = 256  #  int(round(100 * 0.5 / SVSmpp))  # get the patch with 0.5 um/mpp,  must int data type
    ...
    if j < int(w0 / (wholePatch * slidingLength)):
        ...
        for k in range(10):
            ...
            np_slidePatch = cv2.resize(np_slidePatch, (wholePatch, wholePatch))
            # print("Strip2Imags")
            if sum_single_patch < max_threshold and sum_single_patch > min_threshold:

                try:

                    img_Mac = np_slidePatch  # normal_Macenko(np_slidePatch, HERef, maxCRef, Io=255, alpha=1, beta=0.15)
                    temp = [[img_Mac, i, j, k]]
                    result = result + temp
